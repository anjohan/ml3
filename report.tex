...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        \caption{\(\num{10000}\) epochs}
    \end{subfigure}
    \caption{Approximation to the solution of the diffusion equation provided by a neural network with \(6\) hidden layers with \(10\) nodes each. While the network gives horrible results after \(10\) epochs, the solution is seen to approach the analytic solution. As indicated in~\vref{fig:nncost}, however, it seems that the minimisation may have found a local minimum.}
\end{figure}
\begin{figure}
    \centering
    \begin{tikzpicture}
