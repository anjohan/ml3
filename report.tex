...
\begin{abstract}
    ...
    While Forward Euler performed better both in terms of accuracy and runtime, the neural networks also showed promising results, giving a mean squared deviation between the two sides of the diffusion equation on the order of \(10^{-6}\) after \(\num{10000}\) epochs. A combination of the hyperbolic tangent as activation function and the Adam optimiser performed best.
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addlegendentry{\(\exp{-\pi^2 t}\sin(\pi x)\)};
                \addplot+[mark=none] table[y index=4] {data/nn_u_100.dat};
                \addlegendentry{\(t=\num{0.0}\)};
                \addplot+[mark=none] table[y index=5] {data/nn_u_100.dat};
                \addlegendentry{\(t=\num{0.1}\)};
                \addplot+[mark=none] table[y index=6] {data/nn_u_100.dat};
                \addlegendentry{\(t=\num{0.2}\)};
            \end{axis}
        \end{tikzpicture}
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot+[mark=none] table[y index=4] {data/nn_u_10000.dat};
                \addlegendentry{\(t=\num{0.0}\)};
                \addplot+[mark=none] table[y index=5] {data/nn_u_10000.dat};
                \addlegendentry{\(t=\num{0.1}\)};
                \addplot+[mark=none] table[y index=6] {data/nn_u_10000.dat};
                \addlegendentry{\(t=\num{0.2}\)};
            \end{axis}
        \end{tikzpicture}
        \caption{\(u(x,t)=\sin(\pi x)\qty(1-tn(x,t))\),\\ \(\num{10000}\) epochs.}
    \end{subfigure}
    \caption{Approximation to the solution of the diffusion equation provided by a neural network with \(3\) hidden layers with \(100\) nodes each. While the network gives horrible results after \(10\) epochs, the solution is seen to approach the analytic solution, especially for the second choice of approximation. Evidently, it is important to choose a clever transformation of \(n(x,t)\) into a solution which fulfills the boundary and initial conditions.}
\end{figure}
\begin{figure}[H]
    \centering
    ...
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot+[mark=none] table[y index=3] {data/nn_cost.dat};
                \addlegendentry{\(\frac{1}{N_xN_t}\norm{\pdv{u}{t}-\pdv[2]{u}{x}}_2^2\)};
                \addplot+[mark=none] table[y index=4] {data/nn_cost.dat};
                \addlegendentry{\(\frac{1}{N_xN_t}\norm{u - u_\text{exact}}_2^2\)};
            \end{axis}
        \end{tikzpicture}
        \caption{\(u(x,t) = \sin(\pi x)\qty(1 + tn(x,t))\)}
